...
public class Manager extends User implements CSV, List_and_Set, Comparable <Manager> {
    protected String number;
    private String Name[];
    private String Email[];
    private String Number[];
    private Integer Size = 10000000;
    private HashSet <Manager> HashMan;
    private TreeSet <Manager> TreeMan;
    private ArrayList <Manager> ArrayMan;
    private LinkedList <Manager> LinkedMan;
    ...
    public void FillHashSet() {
        Integer i;
        this.HashMan = new HashSet<>(this.Size);
        this.MasOfEmail();
        this.MasOfName();
        this.MasOfNumber();
        for(i = 0; i < this.Size; ++i)
            this.HashMan.add(new Manager(this.Name[i], this.Email[i],this.Number[i]));
    }

    @Override
    public void PrintHashSet() {
        if(!this.HashMan.isEmpty())
            for(Manager p: this.HashMan)
                System.out.println(p.getName() + " " + p.getEmail() + " " + p.getNumber() + "\n");
    ...
    public void FillTreeSet() {
        Integer i;
        //сравнение по всем 3-м полям
        Comparator<Manager> ManComp = new ManagerNameComparator().thenComparing(new ManagerEmailComparator()).thenComparing(new ManagerNumberComparator());
        this.TreeMan = new TreeSet<Manager>(ManComp);
        this.MasOfEmail();
        this.MasOfName();
        this.MasOfNumber();
        for(i = 0; i < this.Size; ++i)
            this.TreeMan.add(new Manager(this.Name[i], this.Email[i],this.Number[i]));
    }

    @Override
    public void PrintTreeSet() {
        if(!this.TreeMan.isEmpty())
            for(Manager p: this.TreeMan)
                System.out.println(p.getName() + " " + p.getEmail() + " " + p.getNumber() + "\n");
        else
    ...
    public void FillArrayList() {
        Integer i;
        this.ArrayMan = new ArrayList<>(this.Size);
        this.MasOfEmail();
        this.MasOfName();
        this.MasOfNumber();
        for(i = 0; i < this.Size; ++i)
            this.ArrayMan.add(new Manager(this.Name[i], this.Email[i],this.Number[i]));
    }

    @Override
    public void PrintArrayList() {
        if(!this.ArrayMan.isEmpty())
            for(Manager p: this.ArrayMan)
                System.out.println(p.getName() + " " + p.getEmail() + " " + p.getNumber() + "\n");
        else
    ...
    public void RemoveArrayList() {
        /*Iterator<Manager> iter = this.ArrayMan.iterator();
        while(iter.hasNext()){
            iter.remove();
        }*/
        this.ArrayMan.clear();
    ...
    public void FillLinkedList() {
        Integer i;
        this.LinkedMan = new LinkedList<>();
        this.MasOfEmail();
        this.MasOfName();
        this.MasOfNumber();
        for(i = 0; i < this.Size; ++i)
            this.LinkedMan.add(new Manager(this.Name[i], this.Email[i],this.Number[i]));
    }

    @Override
    public void PrintLinkedList() {
        if(!this.LinkedMan.isEmpty())
            for(Manager p: this.LinkedMan)
                System.out.println(p.getName() + " " + p.getEmail() + " " + p.getNumber() + "\n");
        else
